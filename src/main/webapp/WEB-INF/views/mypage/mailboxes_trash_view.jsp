        ...
        <div class="con_middle">
            <div class="nav">
                <ul>
                    ...
                    <li><a href="mailboxes_trash_view">휴지통</a></li>
                </ul>
            </div>
            <!-- =================================contents================================================= -->
                ...
                <div style="height: 85.5%">
					<table style="width: 900px; margin: auto;"> 
 						<c:if test="${view.size() == 0 }">
							<tr><td colspan="6" align="center">삭제한 메일이 없습니다.</td></tr>
						</c:if>
						<c:if test="${view.size() != 0 }">
										...
										<td>
											<div style="width:900px;">
												<div class="mailDelete">
 													<button onclick="location.href='mailboxes_service?idx=${mo.idx}&currentPage=${currentPage}&mode=4&category=휴지통'">삭제</button>
 												</div>
												<div class="restore">
 													<button onclick="location.href='mailboxes_service?idx=${mo.idx}&currentPage=${currentPage}&mode=3'">복구</button>
 												</div>
												<div style="margin-top:5px;width:800px;">
													<a href='mailboxes_content_list?idx=${mo.idx}&currentPage=${MailBoxesList.currentPage}&category=메일함'>${mo.title}</a>
												</div>
				...
				<table style="width: 900px; margin: auto;">
					<tr>
						<td colspan="4" align="right">
                			<input type="hidden" value="메일 쓰기" onclick="location.href='mailboxes_insert'"/>
                		</td>
					</tr>
                </table>
                ...
                <table class="pagebutton" align="center" border="0" cellpadding="0" cellspacing="0" height="30">
					<tr>		
						<c:if test="${MailBoxesList.currentPage > 1 }">
							<td><button type="button" title="첫 페이지로" onclick="location.href='?currentPage=1&category=휴지통'"> 처음 </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage <= 1 }">
							<td><button type="button" title="이미 첫 페이지입니다" disabled="disabled"> 처음 </button></td>
						</c:if>
						<c:if test="${MailBoxesList.startPage > 1 }">
							<td><button type="button" title="10페이지 이동" onclick="location.href='?currentPage=${MailBoxesList.currentPage - 10}&category=휴지통'"> << </button></td>
						</c:if>
						<c:if test="${MailBoxesList.startPage <= 1 }">
							<td><button type="button" title="이미 첫 페이지입니다" disabled="disabled"> << </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage > 1}">
							<td><button type="button" title="전 페이지로" onclick="location.href='?currentPage=${MailBoxesList.currentPage - 1}&category=휴지통'"> < </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage <= 1}">
						...
						<c:forEach var="i" begin="${MailBoxesList.startPage}" end="${MailBoxesList.endPage}">
							...
							<c:if test="${i != MailBoxesList.currentPage}">
								<td class='tda' width='30' align='center'><a href='mypage_list?currentPage=${i}&category=휴지통'>${i}</a></td>
							</c:if>
						</c:forEach>	
						
						<c:if test="${MailBoxesList.currentPage < MailBoxesList.totalPage}">
							<td><button type="button" title="다음 페이지로" onclick="location.href='?currentPage=${MailBoxesList.currentPage+1}&category=휴지통'"> > </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage >= MailBoxesList.totalPage}">
						...
						<c:if test="${MailBoxesList.currentPage < MailBoxesList.totalPage}">
							<td><button type="button" title="마지막 페이지로" onclick="location.href='?currentPage=${MailBoxesList.totalPage}&category=휴지통'"> 끝 </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage >= MailBoxesList.totalPage}">
