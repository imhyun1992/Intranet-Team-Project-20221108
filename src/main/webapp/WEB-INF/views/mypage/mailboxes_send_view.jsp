        ...
        <div class="con_middle">
            <div class="nav">
                <ul>
                    ...
                    <li><a href="mypage_list?category=보낸 메일함">보낸 메일함</a></li>
                </ul>
            </div>
            <!-- =================================contents================================================= -->
                ...
                <div style="height: 80%">
					<table style="width: 900px; margin: auto;"> 
 						<c:if test="${view.size() == 0 }">
							<tr><td colspan="6" align="center">보낸 메일이 없습니다.</td></tr>
						</c:if>
						<c:if test="${view.size() != 0 }">
									...
									<tr>
										<td>
											<p style="margin-top:5px;">
												<a href='mailboxes_content_list?idx=${mo.idx}&currentPage=${MailBoxesList.currentPage}&divs=send'>${mo.title}</a>
											</p>
											<p style="padding-bottom:5px;border-bottom:1px solid black;font-size:8px;">
                ...
                <table class="pagebutton" align="center" border="0" cellpadding="0" cellspacing="0" height="30">
					<tr>		
						<c:if test="${MailBoxesList.currentPage > 1 }">
							<td><button type="button" title="첫 페이지로" onclick="location.href='?currentPage=1&category=보낸 메일함'"> 처음 </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage <= 1 }">
							<td><button type="button" title="이미 첫 페이지입니다" disabled="disabled"> 처음 </button></td>
						</c:if>
						<c:if test="${MailBoxesList.startPage > 1 }">
							<td><button type="button" title="10페이지 이동" onclick="location.href='?currentPage=${MailBoxesList.currentPage - 10}&category=보낸 메일함'"> << </button></td>
						</c:if>
						<c:if test="${MailBoxesList.startPage <= 1 }">
							<td><button type="button" title="이미 첫 페이지입니다" disabled="disabled"> << </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage > 1}">
							<td><button type="button" title="전 페이지로" onclick="location.href='?currentPage=${MailBoxesList.currentPage - 1}&category=보낸 메일함'"> < </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage <= 1}">
						...
						<c:forEach var="i" begin="${MailBoxesList.startPage}" end="${MailBoxesList.endPage}">
							...
							<c:if test="${i != MailBoxesList.currentPage}">
 								<td class='tda' width='30' align='center'><a href='mypage_list?currentPage=${i}&category=보낸 메일함'>${i}</a></td>
							</c:if>
						</c:forEach>	
						
						<c:if test="${MailBoxesList.currentPage < MailBoxesList.totalPage}">
							<td><button type="button" title="다음 페이지로" onclick="location.href='?currentPage=${MailBoxesList.currentPage+1}&category=보낸 메일함'"> > </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage >= MailBoxesList.totalPage}">
						...
						<c:if test="${MailBoxesList.currentPage < MailBoxesList.totalPage}">
							<td><button type="button" title="마지막 페이지로" onclick="location.href='?currentPage=${MailBoxesList.totalPage}&category=보낸 메일함'"> 끝 </button></td>
						</c:if>
						<c:if test="${MailBoxesList.currentPage >= MailBoxesList.totalPage}">
